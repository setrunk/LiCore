    ...
    , mImageCrs( DEFAULT_LATLON_CRS )
{
    ...
    if (mTileMatrixSet)
    {
        auto it = mTileMatrixSet->tileMatrices.cbegin();
        for (; it != mTileMatrixSet->tileMatrices.cend(); ++it)
        {
            LodEntry le;
            le.resolution = it.key();
            le.tm = &it.value();
            m_lodEntries.append(le);
        }
        qSort(m_lodEntries);
    }
...
bool QgsWmsProvider::isTiled() const
{
    return m_lodEntries.size() > 0;
}

int QgsWmsProvider::getNumberOfXTilesAtLevel(int level) const
{
    if (mSettings.mXyz)
        return 1 << level;
...
LiRectangle QgsWmsProvider::tileXYToNativeRectangle(int x, int y, int level) const
{
    ...
    double ox = tm->topLeft.x();
    double oy = tm->topLeft.y();
    double res = getResolution(level);

    double tileSpaceX = tm->tileWidth * res;
    double tileSpaceY = tm->tileHeight * res;
...
Cartesian2 QgsWmsProvider::positionToTileXY(const Cartographic &position, int level) const
{
    ...
    auto tm = m_lodEntries[level].tm;
    double res = getResolution(level);
    double spaceX = tm->tileWidth * res;
    double spaceY = tm->tileHeight * res;
...
int QgsWmsProvider::tileWidth() const
{
    return isTiled() ? m_lodEntries.first().tm->tileWidth : 256;
}

int QgsWmsProvider::tileHeight() const
{
    return isTiled() ? m_lodEntries.first().tm->tileHeight : 256;
}

int QgsWmsProvider::minimumLevel() const
{
    return 0;
}

int QgsWmsProvider::maximumLevel() const
{
    return isTiled() ? m_lodEntries.size()-1 : 100;
}

int QgsWmsProvider::computeLevel(const LiRectangle &rectangle) const
{
    ...
    for (int i = 0; i < m_lodEntries.size(); ++i)
    {
        const auto &lodEntry = m_lodEntries.at(i);
        double targetRes = w / lodEntry.tm->tileWidth * 1.5;
        if ( lodEntry.resolution <= targetRes )
        {
            level = i;
            break;
        }
...
QUrl QgsWmsProvider::getTileUrl(int x, int y, int level)
{
    if (mSettings.mXyz)
    {
        if (level >= m_lodEntries.size())
        ...
        auto tm = m_lodEntries.at(level).tm;
        int z = tm->identifier.toInt();
        QString turl = mSettings.mBaseUrl;

//        if ( turl.contains( QLatin1String( "{q}" ) ) )  // used in Bing maps
//            turl.replace( QLatin1String( "{q}" ), _tile2quadkey( x, y, z ) );

        turl.replace( QLatin1String( "{x}" ), QString::number( x ), Qt::CaseInsensitive );

        // inverted Y axis
        if ( turl.contains( QLatin1String( "{-y}" ) ) )
        {
            turl.replace( QLatin1String( "{-y}" ), QString::number( tm->matrixHeight - y - 1 ), Qt::CaseInsensitive );
        }
        else
        {
            turl.replace( QLatin1String( "{y}" ), QString::number( y ), Qt::CaseInsensitive );
        }
    ...
    {
        // KVP
        url = QUrl( mSettings.mIgnoreGetMapUrl ? mSettings.mBaseUrl : getTileUrl() );
        QUrlQuery query;
        ...
        setQueryItem( query, QStringLiteral( "TILEROW" ), QString::number(y) );
        setQueryItem( query, QStringLiteral( "TILECOL" ), QString::number(x) );
    ...
    {
        // REST
        QString turl = mTileLayer->getTileURLs[ mSettings.mImageMimeType ];

        turl.replace( QLatin1String( "{layer}" ), mSettings.mActiveSubLayers[0], Qt::CaseInsensitive );
        turl.replace( QLatin1String( "{style}" ), mSettings.mActiveSubStyles[0], Qt::CaseInsensitive );
        turl.replace( QLatin1String( "{tilematrixset}" ), mTileMatrixSet->identifier, Qt::CaseInsensitive );
        turl.replace( QLatin1String( "{tilematrix}" ), tm->identifier, Qt::CaseInsensitive );
        turl.replace( QLatin1String( "{tilerow}" ), QString::number( y ), Qt::CaseInsensitive );
        turl.replace( QLatin1String( "{tilecol}" ), QString::number( x ), Qt::CaseInsensitive );

        for ( QHash<QString, QString>::const_iterator it = mSettings.mTileDimensionValues.constBegin(); it != mSettings.mTileDimensionValues.constEnd(); ++it )
        {
            turl.replace( "{" + it.key() + "}", it.value(), Qt::CaseInsensitive );
        }
...
bool QgsWmsProvider::isXyzTiles() const
{
    return mSettings.mXyz;
}

double QgsWmsProvider::getResolution(int level) const
{
...
void QgsWmsProvider::setQueryItem( QUrlQuery &query, const QString &item, const QString &value )
{
    query.removeQueryItem( item );
    query.addQueryItem( item, value );
}

void QgsWmsProvider::setFormatQueryItem( QUrlQuery &query )
{
    query.removeQueryItem( QStringLiteral( "FORMAT" ) );
    if ( mSettings.mImageMimeType.contains( '+' ) )
    {
        ...
        query.addQueryItem( "FORMAT", format.toUtf8() );
    }
    else
        setQueryItem( query, QStringLiteral( "FORMAT" ), mSettings.mImageMimeType );
}
...
void QgsWmsProvider::setSRSQueryItem( QUrlQuery &query )
{
    QString crsKey = QStringLiteral( "SRS" ); //SRS in 1.1.1 and CRS in 1.3.0
    if ( mCaps.mCapabilities.version == QLatin1String( "1.3.0" ) || mCaps.mCapabilities.version == QLatin1String( "1.3" ) )
    {
    ...
    setQueryItem( query, crsKey, mImageCrs );
}
