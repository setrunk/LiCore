...
public:
     ...
     *
     */
    QgsWmsProvider( const QString &uri,
                    const QgsDataProvider::ProviderOptions &options = QgsDataProvider::ProviderOptions(),
                    const QgsWmsCapabilities *capabilities = nullptr );
...
private:
    ...
    void setSRSQueryItem( QUrlQuery &query );
...
private:
    ...
    {
        double resolution;
        const QgsWmtsTileMatrix *tm;
        bool operator <(const LodEntry &other) const
        {
            return resolution > other.resolution;
        }
    };

    QList<LodEntry> m_lodEntries;
    ...
    void setFormatQueryItem( QUrlQuery &query );
