...
template <typename T, typename Completed>
QFuture<T> when(QFuture<T> future, Completed onCompleted)
{
    return observe(future).subscribe(onCompleted).future();
}

template <typename T, typename Completed, typename Canceled>
QFuture<T> when(QFuture<T> future, Completed onCompleted, Canceled onCanceled)
{
    return observe(future).subscribe(onCompleted, onCanceled).future();
}
...
//    QEventLoop loop;
//    QFutureWatcher<T> w;
//    QObject::connect(&w, SIGNAL(finished()), &loop, SLOT(quit()));
//    w.setFuture(future);
//    loop.exec();
